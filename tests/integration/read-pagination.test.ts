...
async function probeDb(dsn: string): Promise<boolean> {
  ...
  try {
    await probe.query('SELECT 1 FROM openfga.tuple LIMIT 1')
    return true
  }
  catch {
    return false
...
if (!dbAvailable) {
  console.warn(
    '[openfga integration] OPENFGA_DB_URL unreachable, unset, or migrations not applied — skipping read-pagination tests.',
  )
}
